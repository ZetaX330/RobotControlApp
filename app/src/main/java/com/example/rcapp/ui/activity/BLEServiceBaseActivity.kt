...
import com.example.rcapp.ui.widget.MainBluetoothToolbar
import com.example.rcapp.ui.viewmodel.MainToolbarViewModel
import com.example.rcapp.util.PermissionConstants.REQUEST_CODE_BLUETOOTH
import com.example.rcapp.util.PermissionConstants.REQUEST_CODE_LOCATION
 ...
 *此基类Activity实现了蓝牙服务绑定，权限请求，Toolbar管理等多个Activity共同需要实现的功能
 */
abstract class BLEServiceBaseActivity: AppCompatActivity() {
    ...
    var bluetoothState :LiveData<Boolean> = _bluetoothState
     ...
     *@requestCode 请求标识码
     */
    fun requestBluetoothPermission(permissionRequest: String) {
     ...
     */
    fun updateBluetoothState(state: Int, gatt: BluetoothGatt?) {
        runOnUiThread {
            when (state) {
                //蓝牙关闭时
                ...
                BluetoothProfile.STATE_CONNECTED ->{
                    if (ActivityCompat.checkSelfPermission(
                        ...
                        requestBluetoothPermission(permission.BLUETOOTH_CONNECT)
